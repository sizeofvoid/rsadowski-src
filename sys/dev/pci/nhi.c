...
nhi_attach(struct device *parent, struct device *self, void *aux)
{
	...
	args[0].v_buffer = pw_uuid;
	args[0].length = sizeof(pw_uuid);
	...
	error = aml_evalname(acpi_softc, NULL, "\\_SB_._OSC", 4, args, &res);
	if (error)
		printf("aml_evalname: %d\n", error);
	printf("ACPI PW");
	...
	static uint8_t usb_uuid[16] = {
		0x3a, 0xd1, 0xa0, 0x23, 0xab, 0x26, 0x6c, 0x48,
		0x9c, 0x5f, 0x0f, 0xfa, 0x52, 0x5a, 0x57, 0x5a
	};

	memset(args, 0, sizeof(args));
	args[0].type = AML_OBJTYPE_BUFFER;
	args[0].v_buffer = usb_uuid;
	args[0].length = sizeof(usb_uuid);
	args[1].type = AML_OBJTYPE_INTEGER;
	args[1].v_integer = 1;
	args[2].type = AML_OBJTYPE_INTEGER;
	args[2].v_integer = 3;
	...
	error = nhi_conf_read(sc, ROUTER_CS_0, 0, ROUTER_CS, 0, &data);
	printf("ROUTER_CS_0: 0x%08x, %d\n", data, error);
	error = nhi_conf_read(sc, ROUTER_CS_1, 0, ROUTER_CS, 0, &data);
	printf("ROUTER_CS_1: 0x%08x, %d\n", data, error);
	while (error == 0 && (data & 0xff) != 0) {
		cap = data & 0xff;
		error = nhi_conf_read(sc, cap, 0, ROUTER_CS, 0, &data);
		printf("  0x%2x: 0x%08x, %d\n", cap, data, error);
	}
	error = nhi_conf_read(sc, ROUTER_CS_2, 0, ROUTER_CS, 0, &data);
	printf("ROUTER_CS_2: 0x%08x, %d\n", data, error);
	error = nhi_conf_read(sc, ROUTER_CS_3, 0, ROUTER_CS, 0, &data);
	printf("ROUTER_CS_3: 0x%08x, %d\n", data, error);
	error = nhi_conf_read(sc, ROUTER_CS_4, 0, ROUTER_CS, 0, &data);
	printf("ROUTER_CS_4: 0x%08x, %d\n", data, error);
	error = nhi_conf_read(sc, ROUTER_CS_5, 0, ROUTER_CS, 0, &data);
	printf("ROUTER_CS_5: 0x%08x, %d\n", data, error);
	error = nhi_conf_read(sc, ROUTER_CS_6, 0, ROUTER_CS, 0, &data);
	printf("ROUTER_CS_6: 0x%08x, %d\n", data, error);
	for (i = 0; i < 8; i++) {
		error = nhi_conf_read(sc, ADP_CS_1, i, ADP_CS, 0, &data);
		printf("ADP_CS_1: 0x%08x, %d\n", data, error);
		while (error == 0 && (data & 0xff) != 0) {
			cap = data & 0xff;
			error = nhi_conf_read(sc, cap, i, ADP_CS, 0, &data);
			printf("  0x%2x: 0x%08x, %d\n", cap, data, error);
		}
		error = nhi_conf_read(sc, ADP_CS_2, i, ADP_CS, 0, &data);
		printf("ADP_CS_2: 0x%08x, %d\n", data, error);
	}
#endif
...
nhi_activate(struct device *self, int act)
{
	...
	if (act != DVACT_POWERDOWN)
		return 0;
	if ((boothowto & RB_POWERDOWN) == 0)
		return 0;

	error = nhi_conf_read(sc, ROUTER_CS_5, 0, ROUTER_CS, 1, &data);
	if (error)
		printf("error %d\n", error);
	printf("ROUTER_CS_5: 0x%08x\n", data);

	error = nhi_conf_write(sc, ROUTER_CS_5, 0, ROUTER_CS, 2,
	    data | ROUTER_CS_5_SLP);
	if (error)
		printf("error %d\n", error);
	
	error = nhi_conf_read(sc, ROUTER_CS_5, 0, ROUTER_CS, 3, &data);
	if (error)
		printf("error %d\n", error);
	printf("ROUTER_CS_5: 0x%08x\n", data);
...
//		int i;
		...
		case PDF_HOTPLUG:
			...
			printf("ack adapter %d upg %d\n", adapter, upg);
			cmd[0] = 0;
			cmd[1] = 0;
			cmd[2] = 7 << 0;	/* Event Code (HP_ACK) */
			cmd[2] |= adapter << 8;	/* Event Info */
			cmd[2] |= (upg | 0x2U) << 30;

...
amdpmc_attach(struct device *parent, struct device *self, void *aux)
{
	...
	if (error)
		printf("SMU_MSG_LOG_GETDRAM_ADDR_LO %d\n", error);
	error = amdpmc_send_msg(sc, SMU_MSG_LOG_GETDRAM_ADDR_HI, 0, &addr_hi);
	if (error)
		printf("SMU_MSG_LOG_GETDRAM_ADDR_HI %d\n", error);
	addr = (uint64_t)addr_hi << 32 | addr_lo;
	printf("%s: 0x%016lx\n", sc->sc_dev.dv_xname, addr);
	...
	if (error)
		printf("SMU_MSG_LOG_RESET %d\n", error);
	error = amdpmc_send_msg(sc, SMU_MSG_LOG_START, 0, NULL);
	if (error)
		printf("SMU_MSG_LOG_START %d\n", error);
}
...
amdpmc_smu_log_dump(struct amdpmc_softc *sc)
{
	...
	if (error) {
		printf("SMU_MSG_LOG_DUMP_DATA %d\n", error);
		return;
	}
}
